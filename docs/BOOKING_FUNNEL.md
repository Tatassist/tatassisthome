...
4. On confirmation of Form B, enter the seven-email sequence in `docs/SALES_COPY.md`. Gate entry on `bas_marketing_consent=yes`. Filter out all purchasers of Essentials, Working and Complete. On each purchase, remove the buyer from the acquisition sequence. Before each email, check that the subscriber is active, still has `bas_marketing_consent=yes`, and has no buyer status/tag for any tier. Test these conditions in the actual Kit plan/account before enabling the flow.
5. Emails use the latest stored edition fields: `bas_edition_name`, `bas_edition_price`, `bas_edition_count`, `bas_recommendation_reason`, and `bas_result_url`. A missing-field fallback links to the generic Working System sales page. Repeat submissions update fields; they do not create a historical response log or force re-entry to an already completed sequence.
