...
- `/lp/before-you-quote`: five questions, one at a time, then first name, last name and email. Submitting requests the guide plus booking advice and offers; the form states this plainly with an unsubscribe note. There is no checkbox or guide-only branch.
- `/api/offer-quiz`: server validates all five answers, contact details and origin, saves fields in Kit, then enrolls in the Guide + Tips form. It returns success only after these requests succeed.
...
2. Configure Guide + Tips with the approved Before You Quote PDF and confirmation email. Save its ID as `KIT_BOOKING_NURTURE_FORM_ID`. This is the single destination for all quiz signups; `KIT_BOOKING_FORM_ID` is unused by this quiz.
...
6. Test a fresh signup address. Confirm the correct PDF arrives, all answers and names save, the Guide + Tips form is used, confirmation is honored, and sequence delays and suppression work. Test a buyer and an unsubscribed address too.
