...
if (form) {
  ...
  function render() {
    ...
    $('fq-next').textContent=q ? (step===0 ? 'START MY BOOKING CHECK':'CONTINUE') : 'GET MY FREE GUIDE';
    ...
    if (q) {
      ...
      $('fq-question').innerHTML=`<section class="fq-presubmit-offer" aria-labelledby="fq-presubmit-title"><p class="fq-match-label">OUR RECOMMENDATION</p><h2 id="fq-presubmit-title">Working System · $47</h2><p>Ready for more of the work you want to be booked for? Give your next chapter a system to build on.</p><a class="fq-button" href="/lp/booked-artist?edition=working">EXPLORE THE WORKING SYSTEM →</a><p class="fq-small">One payment. 30-day satisfaction guarantee.</p></section><div class="fq-contact-layout"><div><h2 id="fq-question-title" tabindex="-1">Where should we send your guide?</h2><p class="fq-hint">Get your free copy of Before You Quote.</p><div class="fq-contact-fields"><label>First name<input id="firstName" name="firstName" autocomplete="given-name" maxlength="80" value="${esc(contact.firstName)}" required /></label><label>Last name<input id="lastName" name="lastName" autocomplete="family-name" maxlength="80" value="${esc(contact.lastName)}" required /></label><label class="fq-wide">Email<input id="email" name="email" type="email" autocomplete="email" maxlength="254" value="${esc(contact.email)}" required /></label></div><p class="fq-small">${MARKETING_LABEL} <a href="/privacy" target="_blank" rel="noopener noreferrer">Privacy</a>.</p><div class="fq-honey" aria-hidden="true"><label>Website<input id="website" name="website" tabindex="-1" autocomplete="off" /></label></div></div><img class="fq-contact-cover" src="/lead-magnet/before-you-quote-cover.png" width="420" height="560" alt="Before You Quote free PDF guide" /></div>`;
    }
    error('');
  ...
  form.addEventListener('submit',async event=>{
    ...
    finally{busy=false;$('fq-next').disabled=false;$('fq-back').disabled=false;$('fq-next').textContent='GET MY FREE GUIDE';}
  });
}
